...
def resource_path(relative_path):
    try:
        base_path = sys._MEIPASS  # PyInstaller 환경일 경우
...
def parseFiles_shallow(f):
    if f[1] == "c":
        return (f[0], parse_c_shallow(f[0]), f[1])
    elif f[1] == "java":
        return (f[0], parse_java_shallow(f[0]), f[1])
    elif f[1] == "python":
        return (f[0], parse_python_shallow(f[0]), f[1])
    elif f[1] == "go":
        return (f[0], parse_go_shallow(f[0]), f[1])
    elif f[1] == "javascript":
        return (f[0], parse_js_shallow(f[0]), f[1])

def parseFiles_deep(f):
    if f[1] == "c":
        return (f[0], parse_c_deep(f[0]), f[1])
    elif f[1] == "java":
        return (f[0], parse_java_deep(f[0]), f[1])
    elif f[1] == "python":
        return (f[0], parse_python_deep(f[0]), f[1])
    elif f[1] == "go":
        return (f[0], parse_go_deep(f[0]), f[1])
    elif f[1] == "javascript":
        return (f[0], parse_js_deep(f[0]), f[1])
...
def vuddy_hashing(directory, isAbstraction="on", logger=None, progress_callback=None):
    ...
    timeIn = time.time()
    tupleList = loadSource(directory)
    ...
    for idx, tup in enumerate(tupleList):
        ...
        for funcInst in functionInstanceList:
            ...
            _, absBody = new_abstract(funcInst, absLevel, language)
    ...
    full_result = title_info + json.dumps(listOfHashJsons, ensure_ascii=False)
